...
#if defined(__arm__) || defined(__ARM__) || defined(__ARM_ARCH) || defined(__ARM_NEON)
#define EmbeddedPlatform
#endif
#if defined(EmbeddedPlatform)
#include "ArmDeviceInfo.h"
#endif
...
char g_szHardDriveSerialNumber[256] = { 0 };
char g_szHardDriveModelNumber[256] = { 0 };
char g_szCpuId[256] = { 0 };
char g_szBoardId[256] = { 0 };
int PRINT_DEBUG = false;

static void dump_buffer (const char* title,	const unsigned char* buffer,int len);
static void WriteConstantString(char *entry, char *string) {}
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      else
      {
         ...
         {         
			 ...
			 if (0 == g_szHardDriveSerialNumber[0] &&
						//  serial number must be alphanumeric
			            //  (but there can be leading spaces on IBM drives)
				   (isalnum (serialNumber [0]) || isalnum (serialNumber [19])))
			 {
				 strcpy_s(g_szHardDriveSerialNumber, 256, serialNumber);
				 strcpy_s(g_szHardDriveModelNumber, 256, modelNumber);
				done = TRUE;
			 }
#ifdef PRINTING_TO_CONSOLE_ALLOWED
...
int ReadIdeDriveAsScsiDriveInNT(void)
{
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      // Try to get a handle to PhysicalDrive IOCTL, report failure
      // and exit if can't.
      swprintf(driveName, 256, L"\\\\.\\Scsi%d:", controller);
...
void PrintIdeInfo (int drive, DWORD diskdata [256])
{
   ...
   sprintf_s (bufferSize, 32, "%u", diskdata [21] * 512);
   ...
   char string1 [1000];
   sprintf_s(string1,1000, "Drive%dModelNumber", drive);
   WriteConstantString (string1, modelNumber);

   sprintf_s(string1,1000, "Drive%dSerialNumber", drive);
   WriteConstantString (string1, serialNumber);

   sprintf_s(string1,1000, "Drive%dControllerRevisionNumber", drive);
   WriteConstantString (string1, revisionNumber);

   sprintf_s (string1,1000, "Drive%dControllerBufferSize", drive);
   WriteConstantString (string1, bufferSize);

   sprintf_s (string1,1000, "Drive%dType", drive);
   if (diskdata [0] & 0x0080)
...
#endif //#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
...
int ReadPhysicalDriveInfo(void)
{
    int done = 0;
    ...
    if (fd < 0)
    {
        fprintf(stderr, "open hard disk device failed, make sure you have the permission to read the disk device file.\n");
        return -1;
    }
    ...
    strcpy(g_szHardDriveSerialNumber, (char *)hid.serial_no);
    strcpy(g_szHardDriveModelNumber, (char *)hid.model);
#endif
...
{
   int done = false;
   signed long long id = 0;
   // char string [256];
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
   OSVERSIONINFO version;

   strcpy_s(g_szHardDriveSerialNumber, 256, "");
   ...
   else
   {
      ...
      for (attempt = 0;
		  attempt < 10 && !done && 0 == g_szHardDriveSerialNumber[0];
           attempt++)
         done = ReadDrivePortsInWin9X ();
   }
#elif defined(__linux) || defined(__linux__) || defined(linux)
   ReadPhysicalDriveInfo();
#endif
   if (g_szHardDriveSerialNumber[0] > 0)
   {
	   char *p = g_szHardDriveSerialNumber;

	  WriteConstantString(const_cast<char*>("HardDriveSerialNumber"), g_szHardDriveSerialNumber);

      // ignore first 5 characters from western digital hard drives if
      // the first four characters are WD-W
	  if (!strncmp(g_szHardDriveSerialNumber, "WD-W", 4))
         p += 5;
      for ( ; p && *p; p++)
      {
         if ('-' == *p) 
   ...
   id %= 100000000;
   if (strstr(g_szHardDriveModelNumber, "IBM-"))
      id += 300000000;
   else if (strstr(g_szHardDriveModelNumber, "MAXTOR") ||
	   strstr(g_szHardDriveModelNumber, "Maxtor"))
      id += 400000000;
   else if (strstr(g_szHardDriveModelNumber, "WDC "))
      id += 500000000;
   else
      id += 600000000;
...
int GetCpuID()
{
	// 1. old(use for x32 bit)
	//unsigned long str1, str2;
	//__asm
	//{
	//	mov eax, 01h
	//		xor edx, edx
	...
	//		mov str2, eax
	//}
	//sprintf_s(g_szCpuId, 256, "%08X%08X", str1, str2);

	// 2. use for x32/x64 bit 
#if defined(_WIN32) || defined(__WIN32__) || defined(WIN32)
	INT32 deBuf[4] = { 0 };
	__cpuidex(deBuf, 01, 0);
	sprintf_s(g_szCpuId, 256, "%08X%08X", deBuf[3], deBuf[0]);
#elif defined(__linux) || defined(__linux__) || defined(linux) || defined(__APPLE__)
#ifndef EmbeddedPlatform
    unsigned int s1, s2;
    char sel;
    asm volatile
        ("movl $0x01 , %%eax ;\n\t"
    ...
    sprintf(g_szCpuId, "%08X%08X", htonl(s2), htonl(s1));
#else // EmbeddedPlatform
    DeviceInfo deviceInfo;
    string strDeviceID = deviceInfo.GetDeviceID();

    strcpy(g_szCpuId, strDeviceID.c_str());
#endif // EmbeddedPlatform
#endif // Platform
	return 0;
}
...
#else // EmbeddedPlatform
    // Ignore For EmbeddedPlatform May Failed To Get CPUID and BoardID
#endif // EmbeddedPlatform
#endif
    return (0);
...
void TrimStr2(char * p)
{
	int len = strlen(p);
	int start = 0;
	int end = len;
	while (isspace(p[start++]));
	start--;
	while (isspace(p[--end]));
	strncpy(p, p + start, end - start + 1);
	p[end - start + 1] = '\0';
}
...
FPD_EXPORT int GetHardInfo(char str[256])
{
	...
	//FILE *fp = fopen("C:\\test.log", "w+");
	//fprintf(fp, "org cpuid len:%d", strlen(g_szCpuId));
    string org_cpuid = string((const char *)g_szCpuId, 256);
    string cpuid = StrRemoveSpace(org_cpuid);
    string org_hdsn = string((const char *)g_szHardDriveSerialNumber);
    string hdsn = StrRemoveSpace(org_hdsn);
	sprintf(str,"%s%s", cpuid.c_str(), hdsn.c_str());
	//fprintf(fp, "cpuid:%s, len:%d", cpuid.c_str(), cpuid.length());
	//fprintf(fp, "hdsn:%s, len:%d", hdsn.c_str(), hdsn.length());
	//fflush(fp);
	//fclose(fp);
	return 0;
}
