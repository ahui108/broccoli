...
#endif
#include<iostream>
//#include "asp/utility.h"
//using asp::sysutil::wsnprintf;
...
int ReadPhysicalDriveInNTWithAdminRights (void)
{
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      wsnprintf(driveName, 256, L"\\\\.\\PhysicalDrive%d", drive);
...
int ReadIdeDriveAsScsiDriveInNT(void)
{
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      WCHAR   driveName [256];
         ...
         //  Try to get a handle to PhysicalDrive IOCTL, report failure
         //  and exit if can't.
      wsnprintf(driveName, 256, L"\\\\.\\Scsi%d:", controller);
...
long GetHardDriveComputerID ()
{
   ...
   if (g_szHardDriveSerialNumber[0] > 0)
   {
	  ...
	  WriteConstantString("HardDriveSerialNumber", g_szHardDriveSerialNumber);
...
int GetHardInfo(char str[256])
{
	GetCpuID();
	GetHardDriveComputerID();

	//TrimStr2(strCpuId);
	//TrimStr2(HardDriveSerialNumber);
	//sprintf_s(str, 256, "%s%s", strCpuId, HardDriveSerialNumber);
